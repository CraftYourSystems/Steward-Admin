...
import { z } from "zod";
import {
  Plus, RefreshCw, Pencil, Users, Loader2, Copy, Check, ClipboardList,
} from "lucide-react";
      ...
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        ...
        <div className="flex gap-2 mb-5">
          {(["invite", "password"] as const).map((mode) => (
            ...
            >
              {mode === "invite" ? "Invite by Gmail" : "Create with PIN"}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-success/30 bg-success/10 p-4 space-y-3">
                ...
                <p className="text-[12px] text-fg-muted">
                  Share these details with them so they can sign in:
                </p>
                <div className="rounded-md border border-border bg-surface px-3 py-2.5 space-y-1">
                  <p className="text-[11px] text-fg-subtle uppercase tracking-wide font-medium">
                    Restaurant code
                  </p>
              ...
              <div className="flex gap-2">
                <Button className="flex-1" onClick={() => setInviteSuccess(null)}>Add Another</Button>
          ...
          tempPin ? (
            <div className="space-y-4">
              <div className="rounded-lg border border-success/30 bg-success/10 p-4 space-y-2">
                ...
                <p className="text-[12px] text-fg-muted">
                  Share this 4-digit PIN with the staff member. They enter it on the Staff login tab
                  along with the restaurant code to clock in. It will not be shown again.
                </p>
      ...
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="rounded-lg border border-border bg-surface-2 p-3 space-y-2">
            <p className="text-[12px] font-medium text-fg">Staff PIN</p>
            {resetPinResult ? (
              <div className="space-y-2">
                <p className="text-[11px] text-fg-muted">New PIN generated. Share it with the staff member:</p>
                <div className="flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2">
                  <code className="flex-1 text-[18px] font-bold font-mono tracking-widest text-fg select-all">
                    {resetPinResult}
                  </code>
                  <button
                    type="button"
