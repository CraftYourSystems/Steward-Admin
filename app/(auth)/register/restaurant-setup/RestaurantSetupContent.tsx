...
function decodeJwt(token: string): { email?: string } | null {
  try {
    ...
    if (typeof window !== 'undefined') {
      rawBinary = window.atob(base64);
    } else {
      rawBinary = Buffer.from(base64, 'base64').toString('binary');
    }
...
export default function RestaurantSetupContent() {
  ...
  // Prefill email from the active JWT token if the prefilled state is empty
  useEffect(() => {
    const token = oauthToken || existingToken;
    if (token && !prefillEmail) {
      const decoded = decodeJwt(token);
      if (decoded?.email) {
        setPrefillEmail(decoded.email);
      }
    }
  }, [oauthToken, existingToken, prefillEmail]);
  ...
  useEffect(() => {
    ...
    const handleExchange = async () => {
      try {
        ...
        setOauthToken(token);
        setPrefillEmail(data.data.user?.email ?? '');
        setPrefillName([data.data.user?.firstName, data.data.user?.lastName].filter(Boolean).join(' '));
  ...
  const onSubmit = async (values: SetupForm) => {
    setServerError(null);
    try {
      // Use the OAuth token from a fresh code exchange, OR fall back to the
      // access token already in memory for admins who were redirected here
      // after a previous Google login (e.g. from the dashboard layout guard).
      const tokenToUse = oauthToken || existingToken;

      let emailToSubmit = prefillEmail;
      if (!emailToSubmit && tokenToUse) {
        const decoded = decodeJwt(tokenToUse);
        if (decoded?.email) {
          emailToSubmit = decoded.email;
        }
      }
