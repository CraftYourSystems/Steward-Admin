...
import { OverviewTab } from "@/app/(dashboard)/analytics/OverviewTab";
import { AdvancedTab } from "@/app/(dashboard)/analytics/AdvancedTab";
...
export function BearingsView() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = revenue.data.map((d: any) => [
    ...
    link.setAttribute("download", `steward-analytics-${activeRange}-${new Date().toISOString().split("T")[0]}.csv`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-3 gap-4 border-b border-white/5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="flex items-center gap-1.5 h-8 px-3 rounded-md border border-white/10 bg-white/5 text-[11px] font-semibold text-fg hover:bg-white/10 hover:border-white/20 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center justify-center h-8 w-8 rounded-md border border-white/10 bg-white/5 text-fg-muted hover:text-fg hover:border-white/20 transition-all disabled:opacity-40 cursor-pointer"
          >
