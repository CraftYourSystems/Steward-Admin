...
export function useKanbanColumns() {
  ...
  const newOrders = useMemo(
    () =>
      orders
        .filter((o: KitchenOrder) => o.status === "NEW")
        .sort(byTime),
    [orders, byTime]
  );
  ...
  const preparingOrders = useMemo(
    () =>
      orders
        .filter((o: KitchenOrder) => o.status === "PREPARING")
        .sort(byTime),
    [orders, byTime]
  );
  ...
  const readyOrders = useMemo(
    () =>
      orders
        .filter((o: KitchenOrder) => o.status === "READY")
        .sort(byTime),
    [orders, byTime]
  );
...
export function useKitchenStatusMutation() {
  ...
  return useMutation({
    ...
    async onMutate({ orderId, status }) {
      ...
      queryClient.setQueryData<KitchenOrder[]>(
        ...
        (orders: KitchenOrder[] | undefined) => {
          ...
          if (status === "COMPLETED") {
            return orders.filter((o: KitchenOrder) => o.id !== orderId);
          }
      ...
      queryClient.setQueryData<KitchenOrder[]>(
        ...
        (orders: KitchenOrder[] | undefined) => {
          ...
          if (updatedOrder.status === "COMPLETED") {
            return orders.filter((o: KitchenOrder) => o.id !== updatedOrder.id);
          }
          return orders.map((current: KitchenOrder) =>
            current.id === updatedOrder.id
              ? {
                  ...current,
                  ...updatedOrder,
                  items: current.items.map((item) => {
                    const updatedItem = updatedOrder.items.find((ui: any) => ui.id === item.id);
    ...
    onError(error: unknown, _variables, context) {
      ...
      toast.error(extractApiError(error, "Could not update order"));
    },
  });
}

/**
 * Marks a cash-payment order as paid.
 * Updates the local cache optimistically so the UNPAID badge disappears
 * immediately without waiting for a socket event.
 */
export function useKitchenPayMutation() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (orderId: string) => {
      const { data } = await api.post<ApiSuccess<KitchenOrder>>(
        `/orders/admin/${orderId}/pay`
      );
      return data.data;
    },

    // Optimistic update — flip paymentStatus before the server confirms
    ...
    onError(error: unknown, _orderId, context) {
      if (context?.previousOrders) {
        queryClient.setQueryData(KITCHEN_ORDERS_QUERY_KEY, context.previousOrders);
      }
      toast.error(extractApiError(error, "Could not accept payment"));
    },
  });
}
