...
type RawServerSettings = Omit<RestaurantSettings, "fontFamily" | "menuBgColor" | "menuTextColor"> & {
  ...
  slug?: string | null;
  // backend returns null for these when not set; normaliseSettings guards them to ""
...
function normaliseSettings(raw: Partial<RawServerSettings>): RestaurantSettings {
  ...
  return {
    ...
    menuBgColor:    raw.menuBgColor    ?? DEFAULT_SETTINGS.menuBgColor,
    menuTextColor:  raw.menuTextColor  ?? DEFAULT_SETTINGS.menuTextColor,
    slug: raw.slug ?? undefined,
    // maxDosaCount: guard against null/undefined from older backends — fallback to 8
    maxDosaCount: typeof raw.maxDosaCount === "number" && raw.maxDosaCount > 0
      ? raw.maxDosaCount
      : DEFAULT_SETTINGS.maxDosaCount,
...
function toBackendSettingsPatch(settings: Partial<RestaurantSettings>): Record<string, unknown> {
  ...
  // logoUrl and bannerUrl belong to the Restaurant table (profile endpoint).
  // The /settings Zod schema does not include them — they are silently stripped
  // by Zod if sent, which means the PATCH response won't contain them, and any
  // subsequent normaliseSettings() call would reset them to defaults.
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // First-time setup or no permissions (e.g. Waiters): use safe defaults
