...
import { LayoutGrid, List, UtensilsCrossed, ArrowRight } from "lucide-react";
...
export function TabTheme({ settings, onChange }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* ── Manage menu items link ────────────────────────────────────────────── */}
      <div className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3 mb-2">
        <div className="flex items-center gap-2">
          <UtensilsCrossed className="h-4 w-4 text-fg-subtle" />
          <span className="text-[13px] font-medium text-fg">Menu items & categories</span>
          <span className="text-[12px] text-fg-subtle">— add, edit, and manage availability</span>
        </div>
        <Link
          href="/menu"
          className="flex items-center gap-1 text-[12px] font-medium text-accent hover:text-accent/80 transition-colors"
        >
          Open Menu Manager <ArrowRight className="h-3.5 w-3.5" />
        </Link>
      </div>
