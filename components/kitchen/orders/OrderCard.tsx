...
const ORDER_TYPE_META: Record<OrderType, { icon: React.ReactNode; label: string }> = {
  ...
  COUNTER_PICKUP: { icon: <ShoppingBag className="h-3.5 w-3.5" />, label: "Collect at Counter" },
};
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          ...
          {order.paymentMethod === "cash" && order.paymentStatus !== "paid" && (
            <span className="text-2xs font-bold uppercase tracking-widest text-amber-400 bg-amber-400/10 border border-amber-400/25 px-1.5 py-0.5 rounded-full">
              Unpaid
            </span>
          )}
        </div>
