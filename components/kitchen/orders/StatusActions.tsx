...
import { useKitchenStatusMutation, useKitchenPayMutation } from "@/hooks/useKitchenOrders";
...
export const StatusActions = memo(function StatusActions({ order }: StatusActionsProps) {
  ...
  const { mutate: payMutate, isPending: isPayPending } = useKitchenPayMutation();
  ...
  const handleAcceptCash = useCallback(() => {
    payMutate(order.id);
  }, [order.id, payMutate]);
  ...
  return (
    <div className="flex flex-col gap-2 pt-3 mt-2 border-t border-white/[0.06]">
      {/* Accept Cash button — shown only for unpaid cash orders */}
      {isCashUnpaid && (
        <button
          disabled={isPayPending || isPending}
          onClick={handleAcceptCash}
          className={cn(
            "w-full touch-target flex items-center justify-center gap-2",
            ...
            "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/30",
            "bg-amber-500/15 hover:bg-amber-500/22 active:bg-amber-500/28",
      ...
      <div className="flex gap-2">
        {primaryLabel && (
          <button
            disabled={isPending || isPayPending}
