...
import { aggregateLiveItems } from "@/lib/liveQueue";
import type { KitchenOrder } from "@/types";
import type { AggregatedLiveItem } from "@/lib/liveQueue";
...
export const LiveOrderSection = memo(function LiveOrderSection({
  variant,
  orders,
  categoryId,
  capacity,
  ...
  // Memoize aggregation — only recalculate when orders array or categoryId changes
  const aggregated = useMemo(() => aggregateLiveItems(orders, categoryId), [orders, categoryId]);
...
function EmptySection({ variant }: { variant: "current" | "upcoming" }) {
  const text =
    variant === "current"
      ? "No active orders for this category"
      : "No upcoming orders for this category";
