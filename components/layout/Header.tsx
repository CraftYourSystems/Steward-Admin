...
import {
  Menu, Search, Bell, X, ShoppingBag,
  LogOut, ChevronRight, Loader2, Package, Settings as SettingsIcon,
  Circle,
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="px-3.5 py-3 border-b border-border">
        ...
        <span className="inline-flex items-center rounded-full border border-border bg-surface-2 px-2 py-0.5 text-[10px] font-medium text-fg-muted">
          {roleLabel}
        </span>
      </div>
...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  const initials = user
    ...
    : "";
  const canManageOrders = hasPermission(user?.role, Permissions.ORDER_MANAGEMENT);
  ...
  return (
    <>
      <header className="relative z-30 flex h-12 flex-shrink-0 items-center justify-between border-b border-border bg-bg/80 px-4 backdrop-blur supports-[backdrop-filter]:bg-bg/60">
        ...
        <div className="flex items-center justify-end gap-2 overflow-visible flex-1">
          {/* Notifications */}
          {canManageOrders && (
      ...
      {canManageOrders && searchOpen && <SearchModal onClose={() => setSearchOpen(false)} />}
    </>
  );
}
